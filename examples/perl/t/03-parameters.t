...
is($serialize->("hello \"world\""), '"hello \\"world\\""', 'String parameter double-quotes escaped');
...
ok($serialized_hash eq '{name: "Alice", age: 30}' || $serialized_hash eq '{age: 30, name: "Alice"}', 'Hash serialized correctly');
...
my $res_read = $graph->query(
    "MATCH (p:person) WHERE p.name = \$name RETURN p.age, p.active, p.hobbies",
    { name => "Bob \\O'Connor\"" }
);
