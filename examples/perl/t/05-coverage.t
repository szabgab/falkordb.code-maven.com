...
ok($@ =~ /Invalid graph operation/ || $@ =~ /does not exist/ || $@, 'delete_graph of nonexistent graph throws exception');
...
my $exp_res = $graph->explain("MATCH (p:person) WHERE p.name = \$name RETURN p", { name => 'Alice' });
ok(ref $exp_res eq 'ARRAY', 'explain with parameters returns array ref');

my $prof_res = $graph->profile("MATCH (p:person) WHERE p.name = \$name RETURN p", { name => 'Alice' });
ok(ref $prof_res eq 'ARRAY', 'profile with parameters returns array ref');

# 9. Node fallback defaults
my $node_partial = FalkorDB::Node->new_from_resp([
    ['id', 42],
]);
...
my $edge = FalkorDB::Edge->new(id => 1, type => 'knows', src_node => 2, dest_node => 3, properties => {});
